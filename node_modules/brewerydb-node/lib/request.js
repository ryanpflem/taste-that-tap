...
function Request(endpoint, params, config) {
    config = config || cfg;
        var configData = Object.keys(config);
        console.log('configData.length = '+ configData.length)    
        console.log('config Obj keys = ' + Object.keys(config));
        console.log('config =');
        console.log(config);
    this.endpoint = endpoint;
        console.log('endpoint = ' + endpoint);
    this.params = params;
        var paramsData = Object.keys(params);
        console.log('paramsData.length = '+ paramsData.length)    
        console.log('params Obj keys = ' + Object.keys(params));
        console.log('params =');
        console.log(params);
    this.config = config;
    ...
    request(this.url, function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            if (data.status == "failure") {
                ...
                console.log(currentRequestObj.page);
                currentRequestObj.numPages = data.numberOfPages;
                console.log(currentRequestObj.numPages);
